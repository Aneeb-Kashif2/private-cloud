...
nginx_dir=$(mktemp -d)
cleanup() {
  docker rm -f "$prefix-nginx" "$prefix-backend" "$prefix-frontend" "$prefix-postgres" "$prefix-redis" >/dev/null 2>&1 || true
  docker network rm "$prefix" >/dev/null 2>&1 || true
  rm -rf "$storage_dir" "$nginx_dir"
}
trap cleanup EXIT
...
docker run -d --name "$prefix-nginx" --network "$prefix" -p 127.0.0.1::8080 \
  --mount "type=bind,source=$nginx_dir/default.conf,target=/etc/nginx/conf.d/default.conf,readonly" nginx:stable-alpine >/dev/null
proxy_port=$(docker inspect --format '{{(index (index .NetworkSettings.Ports "8080/tcp") 0).HostPort}}' "$prefix-nginx")
SMOKE_API="http://127.0.0.1:$proxy_port" SMOKE_WEB="http://127.0.0.1:$proxy_port" node scripts/smoke-http.mjs
