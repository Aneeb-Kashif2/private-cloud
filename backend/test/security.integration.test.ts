...
run("security integration", () => {
  ...
  it("creates hashed public shares and enforces download limits",async()=>{const owner=await completedFile("share-owner@example.com","shared.pdf",12);const created=await app.inject({method:"POST",url:`/api/files/${owner.file.id}/shares`,headers:{cookie:owner.header,origin:config.FRONTEND_ORIGIN},payload:{expiration:"never",maxDownloads:1}});expect(created.statusCode).toBe(200);const share=created.json().share as {id:string;token:string};expect(share.token).toMatch(/^[A-Za-z0-9_-]{43}$/);const stored=await prisma.fileShare.findUniqueOrThrow({where:{id:share.id}});expect(stored.tokenHash).not.toBe(share.token);expect(stored.tokenHash).toHaveLength(64);const info=await app.inject({method:"GET",url:`/api/shares/${share.token}`});expect(info.statusCode).toBe(200);expect(info.json().file.originalName).toBe("shared.pdf");expect(info.body).not.toContain(owner.file.id);const download=await app.inject({method:"GET",url:`/api/shares/${share.token}/download`});expect(download.statusCode).toBe(200);expect(download.rawPayload).toEqual(Buffer.alloc(12,65));expect((await app.inject({method:"GET",url:`/api/shares/${share.token}/download`})).statusCode).toBe(404)});
  it("rejects expired shares and prevents another user from revoking them",async()=>{const owner=await completedFile("share-expiry@example.com","expiry.pdf");const created=await app.inject({method:"POST",url:`/api/files/${owner.file.id}/shares`,headers:{cookie:owner.header,origin:config.FRONTEND_ORIGIN},payload:{expiration:"1h"}});const share=created.json().share as {id:string;token:string};await prisma.fileShare.update({where:{id:share.id},data:{expiresAt:new Date(Date.now()-1000)}});expect((await app.inject({method:"GET",url:`/api/shares/${share.token}`})).statusCode).toBe(404);const other=await register("share-other@example.com");expect((await app.inject({method:"DELETE",url:`/api/shares/${share.id}`,headers:{cookie:other.header,origin:config.FRONTEND_ORIGIN}})).statusCode).toBe(404);const active=await app.inject({method:"POST",url:`/api/files/${owner.file.id}/shares`,headers:{cookie:owner.header,origin:config.FRONTEND_ORIGIN},payload:{expiration:"never"}});const activeShare=active.json().share as {id:string;token:string};expect((await app.inject({method:"DELETE",url:`/api/shares/${activeShare.id}`,headers:{cookie:owner.header,origin:config.FRONTEND_ORIGIN}})).statusCode).toBe(204);expect((await app.inject({method:"GET",url:`/api/shares/${activeShare.token}`})).statusCode).toBe(404)});
